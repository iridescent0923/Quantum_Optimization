...
def plot_result(result_data, tau_dephase, gamma_ps_select, object):
    ...
    if object == 'CFI_PS':
        ...
        if (len(result_data[0, :] == 7)):
            for tau_idx, tau_current in enumerate(tau_dephase):
                plt.figure()
                plt.plot(
                    result_data[tau_idx][:,Index.PHI.value], 
                    result_data[tau_idx][:,Index.THETA_1.value], 
                    label = f'$\\tau$ = {tau_current}'
                )
                plt.yticks(
                ...
                plt.plot(
                    result_data[tau_idx][:,Index.PHI.value], 
                    result_data[tau_idx][:,Index.THETA_2.value], 
                    label = f'$\\tau$ = {tau_current}'
                )
                plt.yticks(
                    [-np.pi, -np.pi/2, 0, np.pi/2, np.pi, (3*np.pi)/2, 2*np.pi], 
                ...
                plt.plot(
                    result_data[tau_idx][:,Index.PHI.value], 
                    result_data[tau_idx][:,Index.THETA_3.value], 
                    label = f'$\\tau$ = {tau_current}'
                )
                plt.yticks(
                    [-np.pi, -np.pi/2, 0, np.pi/2, np.pi, (3*np.pi)/2, 2*np.pi], 
        ...
        if (len(result_data[0, :] == 7)):
            for tau_idx, tau_current in enumerate(tau_dephase):
                plt.plot(
                    result_data[tau_idx][:,Index.PHI.value], 
                    result_data[tau_idx][:,Index.TAU_1_3.value], 
                    label = f'$\\tau$ = {tau_current}'
                )
        ...
        if (len(result_data[0, :] == 7)):
            for tau_idx, tau_current in enumerate(tau_dephase):
                plt.plot(
                    result_data[tau_idx][:,Index.PHI.value], 
                    result_data[tau_idx][:,Index.TAU_2_3.value], 
                    label = f'$\\tau$ = {tau_current}'
                )
            plt.yticks(
                [-np.pi, -np.pi/2, 0, np.pi/2, np.pi, (3*np.pi)/2, 2*np.pi], 
